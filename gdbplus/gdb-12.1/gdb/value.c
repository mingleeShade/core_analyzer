...
void value_free_from(size_t pos)
{
    if (pos < 0 || pos >= all_values.size())
        return;
    all_values.erase(all_values.begin() + pos - 1, all_values.end());
}

struct value* value_at(size_t pos)
{
    if (pos < 0 || pos >= all_values.size())
        return nullptr;
    return all_values.at(pos).get();
}
